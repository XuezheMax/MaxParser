...
public class FirstOrderFeatureGenerator extends FeatureGenerator{
	...
	public void addLinearFeatures(String prefix, int[] obsVals, int first, int second, String attachDistance, ParserModel model, FeatureVector fv){
		...
		for(int i = first + 1; i < second; i++){
			addFeature(prefix, feat + obsVals[i], model, fv);
			addFeature(prefix, feat + obsVals[i] + attachDistance, model, fv);
		}
	}
	...
	public void addCorePOSFeatures(String prefix, int[] obsVals, int first, int second, String attachDistance, ParserModel model, FeatureVector fv){
		int leftOf1 = first > 0 ? obsVals[first - 1] : model.getPOSIndex("STR");
		int rightOf1 = first < second - 1 ? obsVals[first + 1] : model.getPOSIndex("MID");
		int leftOf2 = second > first + 1 ? obsVals[second - 1] : model.getPOSIndex("MID");
		int rightOf2 = second < obsVals.length - 1 ? obsVals[second + 1] : model.getPOSIndex("END");
 ...
 **/
	public void addTwoObsFeatures(String prefix, int obs1F1, int obs1F2, int obs2F1, int obs2F2, String attachDistance, ParserModel model, FeatureVector fv){
		StringBuilder feat = new StringBuilder(obs1F1);
		...
		//
		feat = new StringBuilder(obs1F2);
		addFeature(prefix + "7", feat.toString(), model, fv);
		feat.append('&').append(attachDistance);
		addFeature(prefix + "7", feat.toString(), model, fv);
		
		//
		feat = new StringBuilder(obs2F1);
		addFeature(prefix + "8", feat.toString(), model, fv);
		feat.append('&').append(attachDistance);
		addFeature(prefix + "8", feat.toString(), model, fv);
		
		//
		feat = new StringBuilder(obs2F2);

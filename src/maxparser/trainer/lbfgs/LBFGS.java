...
public class LBFGS {
	...
	public static void lbfgs(int n, int m, double[] x, double f, double[] g,
		...
		while (true) {
			...
			if (execute_entire_while_loop || iflag[0] == 2) {
				if (iter != 1) {
						...
						for (i = 1; i <= n; ++i) {
							if (diag[i - 1] <= 0) {
								...
								throw new ExceptionWithIflag(
										iflag[0], "The " + i + "-th diagonal element of the inverse hessian approximation is not positive.");
							}
						}
					}
					cp = point;
					...
					for (i = 1; i <= n; ++i) {
						w[i - 1] = -g[i - 1];
						if (badNum(w[i - 1]))
							throw new IllegalArgumentException(i + " " + g[i - 1]);
					}
			...
			double directionMag = ddot(n, w, ispt + point * n, 1, w, ispt + point * n, 1);
			double effectiveStep = stp[0] * directionMag;
			if (effectiveStep > MY_MAX_STEP) {
				if (iprint[0] > 0)
					System.err.println("reducing step: " + stp[0] + " => " + (MY_MAX_STEP / directionMag));
			...
			Mcsrch.mcsrch(n, x, f, g, w, ispt + point * n, stp, ftol, xtol, maxfev, info, nfev, diag, iprint);
	...
	public static void lb1(int[] iprint, int iter1, int nfun1, double gnorm1,
			int n, int m, double[] x, double f, double[] g, double[] stp11, boolean finish1) {
		...
		if (iter1 == 0) {
			...
			if (iprint[1 - 1] != 0) {
				...
				if (iprint[2 - 1] > 1 && finish1)
					System.err.println(heading);
				System.err.println("\t" + iter1 + "\t" + nfun1 + "\t" + f + "\t" + gnorm1 + "\t" + stp11[0]);
			}
